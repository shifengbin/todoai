...
系统 SHALL 允许用户从单个工程目录或父目录导入一个或多个全局项目候选。单个工程目录导入 SHALL 将所选目录保存为全局候选项目。父目录导入 SHALL 遍历父目录下的直接子目录，并将可访问的子目录保存为全局候选项目。导入 SHALL 按规范化绝对路径去重；已存在的路径 SHALL 不创建重复候选。

#### Scenario: User imports a single global candidate project

- **WHEN** 用户从创建 TODO、编辑 TODO 或添加工程弹窗选择工程目录 `/repo/frontend-app`
- **THEN** 全局候选项目库包含 `frontend-app`
- **AND** 候选项目路径为 `/repo/frontend-app`

#### Scenario: User imports global candidates from parent directory

- **WHEN** 用户从添加工程弹窗选择父目录 `/repo`
- **AND** `/repo` 下包含可访问目录 `/repo/frontend-app`
- **AND** `/repo` 下包含可访问目录 `/repo/api-service`
- **THEN** 全局候选项目库包含 `frontend-app`
